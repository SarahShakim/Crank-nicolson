...
if const <0.5
    ...
    if a == 1
        ...
        final_crank = final_crank + crank_middle + crank_super + crank_sub;
        ...
        for x = 1:nt - 1
            ...
            if(x >= 2)
                ...
                crank_1 = 2.*z;
                crank_2 = r.*diff(y, 2);
                crank_3 = zeros(nx-2, 1);
    %             crank_known_3(1) = r.*U_out(1, x);
                crank_3(end) = r.*U_out(end, x);
 
                crank_known = crank_1 + crank_2 + crank_3;
                final_crank;
 
                z_new = final_crank\crank_known;
                U_out(2:end-1, x+1) =  z_new;
                U_out(1, x+1) = (4/3).*U_out(2, x+1) - (1/3).*U_out(3, x+1);
